...
pub mod mandelbrot {
    ...
    pub fn generate_window(
        ...
        if current != *past_mandelbrot {
            ...
            let distance = (
                past_mandelbrot.loc.0 + -current_loc.0,
                past_mandelbrot.loc.1 + -current_loc.1,
            );

            println!(
                "past loc: {:?}, current loc: {:?}\n , distance: {:?}",
                past_mandelbrot.loc, current_loc, distance,
            );
            

            for x in (-screen_width/* / 2 */)..(screen_width/* / 2 */) {
                for y in (-screen_height/* / 2 */)..(screen_height/* / 2 */) {
                    let centered_x = (x as i32) + current_loc.0;
                    let centered_y = (y as i32) + current_loc.1;

                    if (centered_x < screen_width - 10 && centered_x > 10)
                        && (centered_y < screen_height - 10 && centered_y > 10)
                    {
                        draw(x, y, canvas, current_loc, distance, zoomed_divisor)?;
                    }
                }
            }
        }
    ...
    fn draw(
        ...
        if is_in_set(Complex::new(
            ...
            canvas.draw_point((x + mouse_loc.0 + distance.0, y + mouse_loc.1 + distance.1))?;
        }
        Ok(())
    }
}
